...
class Model():
    ...
    def build(self):
        ...
        # self.model.addConstrs((
        #         self.arcFlows['J3','Teddington Environmental Flow',k,t] >= 700 \
        #             for k in ['water']
        #             for t in self.timesteps
        #             if t>1 and ,'demand') variables['res_control_curve_l1'][time_reference[t]-1]

                #
                # #IF reservoir below L1 restriction - TEFLOW >= 700
                # if current_storage < :
                #     self.model.addConstrs((
                #         self.arcFlows.sum('*','Teddington Environmental Flow','water',t) >= 700),'demand')
                # # IF reservoir below L2 restriction - TEFLOW >= 300
                # elif current_storage < variables['res_control_curve_l2'][current_month-1]:
                #     self.model.addConstrs((
                #         self.arcFlows.sum('*','Teddington Environmental Flow','water',t) >= 300),'demand')
                # elif current_storage < variables['res_control_curve_l3'][current_month-1]:
                #     self.model.addConstrs((
                #         self.arcFlows.sum('*','Teddington Environmental Flow','water',t) >= 168),'demand')
        ...
        #         self.model.addConstr(
        #             self.arcFlows.sum('*','Teddington Environmental Flow','water',t) >= 168,'TedFlow')
        ...
        # #                     for j in ['Farmoor']
        # #                     for k in ['water']
        # #                     for t in ref_timesteps) <= 55312,'FarmCap')
